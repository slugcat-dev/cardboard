...
function getContext(node: SyntaxNode, doc: Text) {
	...
	for (let i = nodes.length - 1; i >= 0; i--) {
		const node = nodes[i]; let match
		const line = doc.lineAt(node.from); const startPos = node.from - line.from
		if (node.name === 'FencedCode')
			...
			let after = match[3]; let len = match[0].length
		...
		&& (match = /^( *)([-+*])( {1,4}\[[ x]\])?( +)/i.exec(line.text.slice(startPos)))) {
			let after = match[4]; let len = match[0].length
			if (after.length > 4) { after = after.slice(0, after.length - 4); len -= 4 }
			let type = match[2]
			if (match[3])
				type += match[3].replace(/x/i, ' ')
...
export const insertNewlineContinueMarkup: StateCommand = ({ state, dispatch }) => {
	const tree = syntaxTree(state); const { doc } = state
	let dont = null; const changes = state.changeByRange((range) => {
		if (!range.empty || !markdownLanguage.isActiveAt(state, range.from))
			return dont = { range }
		const pos = range.from; const line = doc.lineAt(pos)
		...
		if (inner.item && emptyLine) {
			const first = inner.node.firstChild!; const second = inner.node.getChild('ListItem', 'ListItem')
			// Not second item or blank line before: delete a level of markup
			if (first.to >= pos || second && second.to < pos
				|| line.from > 0 && !/[^\s>]/.test(doc.lineAt(line.from - 1).text)) {
				const next = context.length > 1 ? context[context.length - 2] : null
				let delTo; let insert = ''
				...
				return { range: EditorSelection.cursor(pos + insert.length + 1), changes: { from: line.from, insert: insert + state.lineBreak } }
			}
		}

		if (inner.node.name === 'Blockquote' && emptyLine && line.from) {
			const prevLine = doc.lineAt(line.from - 1); const quoted = />\s*$/.exec(prevLine.text)
...
function nonTightList(node: SyntaxNode, doc: Text) {
	if (node.name !== 'OrderedList' && node.name !== 'BulletList')
		return false
	const first = node.firstChild!; const second = node.getChild('ListItem', 'ListItem')
	if (!second)
		return false
	const line1 = doc.lineAt(first.to); const line2 = doc.lineAt(second.from)
...
function contextNodeForDelete(tree: Tree, pos: number) {
	let node = tree.resolveInner(pos, -1); let scan = pos
	if (isMark(node)) {
		scan = node.from
		node = node.parent!
	}
...
export const deleteMarkupBackward: StateCommand = ({ state, dispatch }) => {
	...
	let dont = null; const changes = state.changeByRange((range) => {
		const pos = range.from; const { doc } = state
		if (range.empty && markdownLanguage.isActiveAt(state, range.from)) {
